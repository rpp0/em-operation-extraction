...
def dense_relu(inputs, num_inputs, num_outputs, stddev=1.0):
    W = tf.Variable(tf.random_normal([num_inputs, num_outputs], stddev=stddev))
    b = tf.Variable(tf.random_normal([num_outputs], stddev=stddev))
    return tf.nn.relu(tf.add(tf.matmul(inputs, W), b))
